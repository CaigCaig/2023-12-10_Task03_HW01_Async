...
#ifdef NO_PROMISE
int search_min(vector<int> data, int i)
{
	auto val = i;
...
vector<int> selection_sort(vector<int> arr, int size)
{
	...
	{
		auto i_swap = async(search_min, arr, i);
...
#else
int search_min(vector<int> data, int i, promise<int>prom)
{
	auto val = i;
	for (auto j = i + 1; j < data.size(); j++) if (data[j] < data[val]) val = j;
	prom.set_value(val);
...
vector<int> selection_sort(vector<int> arr, int size)
{
	...
	for (auto i = 0; i < size; ++i)
	{
		auto ft = async(search_min, arr, i, move(prom));
		swap(arr[i], arr[i_swap.get()]);
	}
	return arr;
}
